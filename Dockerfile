...
RUN apt-get update \
    ...
    && sed -i 's@files = /etc/supervisor/conf.d/*.conf@; files = /etc/supervisor/conf.d/*.conf@' /etc/supervisord.conf \
    && echo 'files = /shareVolume/config/supervisor/*.ini' > /etc/supervisord.conf \
