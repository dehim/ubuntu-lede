...
RUN apt-get update \
    && apt-get install -y apt-utils dialog openssh-server openssl vim tzdata sudo xz-utils iputils-ping supervisor time libjpeg-dev \
    ...
    && addgroup openwrt \
    && useradd -g openwrt -r -m -s /bin/bash openwrt \
    && echo 'openwrt:openwrt' |chpasswd \
    && echo 'root:root' |chpasswd \
    && echo "openwrt ALL=(ALL:ALL) ALL \nopenwrt ALL=(ALL:ALL) NOPASSWD:ALL" > /etc/sudoers.d/default \
    ...
    && touch /home/openwrt/.Xauthority \
    && chown openwrt:openwrt /home/openwrt/.Xauthority \
	&& ln -s /home/openwrt/.Xauthority /root/.Xauthority \
    ...
    && mkdir -p /shareVolume_demo/openwrt/ \
    && chown -R openwrt:openwrt /shareVolume_demo/openwrt \
